...
Function IntuneAppValues
{
	...
	$IntuneAppValues = @{}
    $IntuneAppValues.Add("AppName","Desktop Company Files-v147")
    $IntuneAppValues.Add("AppInstaller","ps1")
    $IntuneAppValues.Add("AppInstallName","Desktop Company Files.ps1")
    $IntuneAppValues.Add("AppInstallArgs","ARGS:-mode C")
    ...
    $IntuneAppValues.Add("AppVar1","Company Files|D3204ECEF813DAD03770B52137722A1F")
...
$customps1_injection_lines +='#>'
$customps1_injection_lines +=''
$customps1_injection_lines +='Function GetHashOfFiles ($FilePaths, $ByDateOrByContents="ByContents")'
$customps1_injection_lines +='{'
$customps1_injection_lines +='    # GetHashOfFiles - Give a list of filepaths, creates an MD5 hash of their content (ByContents) or their date time stamps (ByDate)'
$customps1_injection_lines +='    # $sErr,$sHash = GetHashOfFiles $FilePaths ByDateOrByContents "ByDate"'
$customps1_injection_lines +='	#'
$customps1_injection_lines +='	# Sort so that files are always sorted the same across systems and cultures and powershells. Do this by sorting by the UTF8 byte array of each path.'
$customps1_injection_lines +='    #$FilePaths = $FilePaths | Sort-Object {[system.Text.Encoding]::UTF8.GetBytes($_)|ForEach-Object ToString X2}'
$customps1_injection_lines +='    $FilePaths = $FilePaths | Sort-Object {[system.Text.Encoding]::UTF8.GetBytes($_)}'
$customps1_injection_lines +='    $sErr = "OK"'
$customps1_injection_lines +='    $HashList = @()'
$customps1_injection_lines +='    ForEach ($FilePath in $FilePaths)'
$customps1_injection_lines +='    { # each file'
$customps1_injection_lines +='        If (Test-Path $FilePath -PathType Leaf)'
$customps1_injection_lines +='        { # file exists'
$customps1_injection_lines +='            $Filethis = Get-ChildItem $FilePath'
$customps1_injection_lines +='            # create object for results'
$customps1_injection_lines +='            $entry_obj=[pscustomobject][ordered]@{'
$customps1_injection_lines +='                Name          = $Filethis.Name'
$customps1_injection_lines +='                LastWriteTime = $Filethis.LastWriteTime.ToUniversalTime().Tostring(''yyyy-MM-dd HH:mm'') # was yyyy-MM-dd hh:mm:ss but OneDrive changes ss by 1 as files are copied in out of OneDrive - also hh was 12 not 24'
$customps1_injection_lines +='                Length        = $Filethis.Length'
$customps1_injection_lines +='                HashType      = $ByDateOrByContents'
$customps1_injection_lines +='                Hash          = ""'
$customps1_injection_lines +='                Fullpath      = $FilePath'
$customps1_injection_lines +='                }'
$customps1_injection_lines +='            if ($ByDateOrByContents -eq "ByContents")'
$customps1_injection_lines +='            { # ByContents (slower)'
$customps1_injection_lines +='                $entry_obj.Hash = (Get-FileHash $FilePath -Algorithm MD5).Hash'
$customps1_injection_lines +='            }'
$customps1_injection_lines +='            else'
$customps1_injection_lines +='            { # ByDate (date can be faked)'
$customps1_injection_lines +='                $entry_obj.Hash ="$($entry_obj.name)|$($entry_obj.LastWriteTime)|$($entry_obj.Length)"'
$customps1_injection_lines +='            }'
$customps1_injection_lines +='            ### append object'
$customps1_injection_lines +='            $HashList+=$entry_obj'
$customps1_injection_lines +='        } # file exists'
$customps1_injection_lines +='        Else'
$customps1_injection_lines +='        { # file not found'
$customps1_injection_lines +='            $sErr = "ERR: File not found: $($FilePath)"'
$customps1_injection_lines +='        } # file not found'
$customps1_injection_lines +='    } # each file'
$customps1_injection_lines +='    # get a hash of all the strings'
$customps1_injection_lines +='    $Hashstring= $HashList.Hash -join ", "'
$customps1_injection_lines +='    $sHash= (Get-FileHash  -Algorithm MD5 -InputStream ([IO.MemoryStream]::new([char[]]$Hashstring))).Hash'
$customps1_injection_lines +='    Return $sErr,$sHash,$HashList'
$customps1_injection_lines +='}'
$customps1_injection_lines +='#  AppVar1 format:'
$customps1_injection_lines +='#  Company Files (Folder1)|511CA236B813559BF3485CBFEEC05599*Company Files (Folder2)|511CA236B813559BF3485CBFEEC05599'
$customps1_injection_lines +='#  AppVar2 format:'
$customps1_injection_lines +='#  Anti-spam policies - Microsoft Defender.url*DeleteMe Folder'
$customps1_injection_lines +='$app_detected = $false'
$customps1_injection_lines +='$FolderPubDesktop=[System.Environment]::GetFolderPath("CommonDesktopDirectory")'
$customps1_injection_lines +='$SourceHashes = @()'
$customps1_injection_lines +='if (($IntuneApp.AppVar1 -match "|") -and ($null -ne $IntuneApp.AppVar1) -and ($IntuneApp.AppVar1 -ne "")) {'
$customps1_injection_lines +='    $SourceHashes = $IntuneApp.AppVar1 -split "\*"'
$customps1_injection_lines +='} # has appvar1'
$customps1_injection_lines +='$Removes = @()'
$customps1_injection_lines +='if (($IntuneApp.Appvar2 -ne "") -and ($null -ne $IntuneApp.AppVar2) ){'
$customps1_injection_lines +='    $Removes = $IntuneApp.Appvar2 -split "\*"'
$customps1_injection_lines +='} # has appvar2'
$customps1_injection_lines +='WriteLog "-----------------------------------------------------"'
$customps1_injection_lines +='WriteLog "Detecting company files on Public Desktop"'
$customps1_injection_lines +='$bMatchAll = $true'
$customps1_injection_lines +='$count = 0'
$customps1_injection_lines +='Foreach ($SourceHash in $SourceHashes) {'
$customps1_injection_lines +='    # region: parse source name and hash'
$customps1_injection_lines +='    $SourceNameHash = $SourceHash -split "\|"'
$customps1_injection_lines +='    $SourceHash = @{'
$customps1_injection_lines +='        SourceName = $SourceNameHash[0]'
$customps1_injection_lines +='        Hash = $SourceNameHash[1]'
$customps1_injection_lines +='    }'
$customps1_injection_lines +='    # endregion: parse source name and hash'
$customps1_injection_lines +='    WriteLog "$((++$count))) " -NoNewline'
$customps1_injection_lines +='    WriteLog $SourceHash.SourceName -ForegroundColor Blue -NoNewline'
$customps1_injection_lines +='    $TargetFiles = Get-ChildItem "$($FolderPubDesktop)\$($SourceHash.SourceName)" -File -Recurse'
$customps1_injection_lines +='    $sErr,$sHash,$HashList = GetHashOfFiles $TargetFiles.FullName -ByDateOrByContents "ByDate"'
$customps1_injection_lines +='    WriteLog " Files:" -NoNewline'
$customps1_injection_lines +='    WriteLog $TargetFiles.Count -ForegroundColor Blue -NoNewline'
$customps1_injection_lines +='    WriteLog " Hash:" -NoNewline'
$customps1_injection_lines +='    WriteLog $sHash -ForegroundColor Blue -NoNewline'
$customps1_injection_lines +='    if ($sHash -eq $SourceHash.Hash) {'
$customps1_injection_lines +='        WriteLog " OK" -ForegroundColor Blue'
$customps1_injection_lines +='    } # hash match'
$customps1_injection_lines +='    else {'
$customps1_injection_lines +='        WriteLog " MISMATCH" -ForegroundColor Yellow'
$customps1_injection_lines +='        $bMatchAll = $false'
$customps1_injection_lines +='    } # hash mismatch'
$customps1_injection_lines +='} # each source hash'
$customps1_injection_lines +='# Delete Checking'
$customps1_injection_lines +='$bDeletesOK = $true'
$customps1_injection_lines +='If ($Removes.Count -gt 0) {'
$customps1_injection_lines +='    WriteLog "$(Split-Path $FilesToRemoveCSV -Leaf)"'
$customps1_injection_lines +='    $count=0'
$customps1_injection_lines +='    Foreach ($Remove in $Removes) {'
$customps1_injection_lines +='        WriteLog "$((++$count))) " -NoNewline'
$customps1_injection_lines +='        WriteLog $Remove -ForegroundColor Blue -NoNewline'
$customps1_injection_lines +='        $PathToRemove = "$($FolderPubDesktop)\$($Remove)"'
$customps1_injection_lines +='        if (Test-Path $PathToRemove) {'
$customps1_injection_lines +='            $bDeletesOK = $false'
$customps1_injection_lines +='            WriteLog " [Err, File found]" -ForegroundColor Yellow'
$customps1_injection_lines +='        } # found'
$customps1_injection_lines +='        else {'
$customps1_injection_lines +='            WriteLog " [OK, Missing]" -ForegroundColor Blue'
$customps1_injection_lines +='        } # missing'
$customps1_injection_lines +='    } # each remove'
$customps1_injection_lines +='} # has removes'
$customps1_injection_lines +='WriteLog "Detect Result: " -NoNewline'
$customps1_injection_lines +='if ($bMatchAll -and $bDeletesOK) {'
$customps1_injection_lines +='    WriteLog "OK: All files detected. No removals needed." -ForegroundColor Blue'
$customps1_injection_lines +='    $app_detected = $true'
$customps1_injection_lines +='} # all good'
$customps1_injection_lines +='else {'
$customps1_injection_lines +='    $ErrMsg = @()'
$customps1_injection_lines +='    if (-not $bMatchAll) {'
$customps1_injection_lines +='        $ErrMsg += "Not all files matched."'
$customps1_injection_lines +='    }'
$customps1_injection_lines +='    if (-not $bDeletesOK) {'
$customps1_injection_lines +='        $ErrMsg += "Not all FilesToRemove are deleted."'
$customps1_injection_lines +='    }'
$customps1_injection_lines +='    WriteLog "Err: $($ErrMsg -join '' '')" -ForegroundColor Yellow'
$customps1_injection_lines +='    $app_detected = $false'
$customps1_injection_lines +='} # has mismatches'
