...
Function IntuneAppValues
{
	...
	$IntuneAppValues = @{}
    $IntuneAppValues.Add("AppName","Adobe Reader-v196")
    ...
    $IntuneAppValues.Add("AppUninstallVersion","22")
    $IntuneAppValues.Add("AppUninstallProcess","Acrobat")
...
$customps1_injection_lines +='# warning: if the package itself is already installed, always show requirements_met, otherwise all installs will result in not applicable status'
$customps1_injection_lines +='# '
$customps1_injection_lines +='$app_to_find     = "Adobe Acrobat*"'
$customps1_injection_lines +='$app_ver_atleast = "0"'
$customps1_injection_lines +='#logic here is that if you have any other Adobe Acrobat product then you don''t meet requirements to install or uninstall Adobe Acrobat Reader'
$customps1_injection_lines +='#so that translates to starts with Adobe Acrobat but isn''t Adobe Acrobat Reader'
...
$customps1_injection_lines +='		#}'
$customps1_injection_lines +='		#Else'
$customps1_injection_lines +='		#{ # this is a different app'
$customps1_injection_lines +='			if ($app_found.Name -like "Adobe Acrobat Reader*")'
$customps1_injection_lines +='			{ # but it''s still Reader'
$customps1_injection_lines +='				WriteLog "$($app_found.name) v$($app_found.version): Installed (is some version of Reader)"'
$customps1_injection_lines +='			}'
$customps1_injection_lines +='			else'
$customps1_injection_lines +='			{ # not Reader'
$customps1_injection_lines +='				WriteLog "$($app_found.name) v$($app_found.version): This app blocks the install of Reader"'
$customps1_injection_lines +='				if ($requirements_met) {$requirements_met = $false}'
$customps1_injection_lines +='			}'
$customps1_injection_lines +='		#} # this is a different app'
$customps1_injection_lines +='    } # version ok'
$customps1_injection_lines +='} # each found app'
$customps1_injection_lines +='WriteLog "requirements_met (after): $($requirements_met)"'
