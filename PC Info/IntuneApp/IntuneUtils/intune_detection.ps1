...
Function IntuneAppValues
{
	...
	$IntuneAppValues = @{}
    $IntuneAppValues.Add("AppName","PC Info-v170")
    $IntuneAppValues.Add("AppInstaller","ps1")
    $IntuneAppValues.Add("AppInstallName","PC Info Setup.ps1")
    $IntuneAppValues.Add("AppInstallArgs","ARGS:-mode install_system")
    $IntuneAppValues.Add("AppUninstallName","")
    $IntuneAppValues.Add("AppUninstallVersion","152")
...
$customps1_injection_lines +='delete this file from your package if it is not needed'
$customps1_injection_lines +=''
...
$customps1_injection_lines +='$app_detected may already be true if regular detection code already found something'
...
$customps1_injection_lines +='$folder_common = "$([Environment]::GetFolderPath("ProgramFiles"))\PC Info"'
$customps1_injection_lines +='$Filechecks +="$folder_common\PC Info Setup.ps1"'
$customps1_injection_lines +='$Filechecks +="$folder_common\PC Info.ps1"'
$customps1_injection_lines +='$Filechecks +="$folder_common\PC Info.lnk"'
$customps1_injection_lines +='$bOK = $true # Assume OK'
...
$customps1_injection_lines +='    WriteLog "File check $($i): $($Filecheck)" -NoNewline'
$customps1_injection_lines +='    if (Test-Path $Filecheck -PathType Leaf) {'
$customps1_injection_lines +='        WriteLog " OK"'
$customps1_injection_lines +='    }'
$customps1_injection_lines +='    else {'
$customps1_injection_lines +='        WriteLog " NOT FOUND"'
$customps1_injection_lines +='        $bOK = $false'
$customps1_injection_lines +='        break'
$customps1_injection_lines +='    }'
$customps1_injection_lines +='} # Each file'
$customps1_injection_lines +='if ($bOK)'
$customps1_injection_lines +='{ # registry'
$customps1_injection_lines +='    $regkeymain = "HKLM:\Software\Microsoft\Active Setup\Installed Components\PC Info"'
$customps1_injection_lines +='    $regkeyname = "Version"'
$customps1_injection_lines +='    $regkeyvalue = "2"'
$customps1_injection_lines +='    $regvalue = Get-ItemProperty -Path $regkeymain -Name $regkeyname -ErrorAction SilentlyContinue | Select-Object -ExpandProperty $regkeyname'
$customps1_injection_lines +='    if ($regvalue -eq $regkeyvalue) {'
$customps1_injection_lines +='        WriteLog "Reg check: $($regkeymain)\$($regkeyname) is set to $($regkeyvalue) OK"'
$customps1_injection_lines +='    } else {'
$customps1_injection_lines +='        WriteLog "Reg check: $($regkeymain)\$($regkeyname) is set to $($regkeyvalue) NOT FOUND"'
$customps1_injection_lines +='        $bOK = $false'
$customps1_injection_lines +='    }'
$customps1_injection_lines +='} # registry'
$customps1_injection_lines +=''
