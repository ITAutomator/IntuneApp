...
write-host "v2.1"
write-host ""
write-host "Replaces files with $($zip_ext) files (using 7zip)"
write-host ""
write-host ""
...
$entries = Get-ChildItem $folder -File -Include $includes -Exclude $excludes -Recurse
    ...
    foreach ($x in $entries)
    {
        ...
        if ($choiceRTN -eq 2) 
            ...
            if (($choiceLoop -eq 0) -or ($choiceLoop -eq 1))
                ...
                $target = $x.FullName + "." + $zip_ext
                If ((Test-Path $target) -or (Test-Path ($target+".001")))
                    {
                    $date = get-date -format "yyyy-MM-dd_HH-mm-ss"
                    $target_rename = $target = $x.FullName + "."+ $date + "." + $zip_ext
                    #
                    write-host "  Target collison found, using : $($target_rename)"
                    #Rename-Item $target -NewName $target_rename
                    }
                ####################
                <#
                ...
                If (Test-Path $target)
                    ...
                    Do
                        {
                        $i+=1
                        $target_rename = $target + ".v" + $i
                        }
                    Until (!(Test-Path $target_rename))
                    write-host "  Collison found - renaming : $($target_rename)"
                    Rename-Item $target -NewName $target_rename
                    }
                ####################
                #>
