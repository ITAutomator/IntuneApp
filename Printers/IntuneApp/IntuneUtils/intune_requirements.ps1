...
Function IntuneAppValues
{
	...
	$IntuneAppValues = @{}
    $IntuneAppValues.Add("AppName","Printers-v143")
...
$customps1_injection_lines +='$true indicates requirements are met, $false indicates requirements are not met.'
$customps1_injection_lines +='If requiremeents are met, the app will be considered installable and the setup script will run.'
$customps1_injection_lines +='If requiremeents are not met, the machine will be considered inapplicable for the app to be installed.'
$customps1_injection_lines +=''
...
$customps1_injection_lines +='This is because requirements checking gets tripped up by WriteLog (and write-output) so only use WriteLog for informational display.'
$customps1_injection_lines +='Do not allow Write-Output or unintentional ouput, other than the return value.'
$customps1_injection_lines +='This must be a stand-alone script with no local files available. It will be copied to a temp folder and run under system context.'
...
$customps1_injection_lines +='To debug this script, put a break in the script and run the parent ps1 file (Requirements).'
...
$customps1_injection_lines +=''
$customps1_injection_lines +='<#'
$customps1_injection_lines +='#region Check if app installed'
$customps1_injection_lines +='#'
$customps1_injection_lines +='# checks for conflicting app.  if no app preset, it''s OK to install here'
$customps1_injection_lines +='# this sample code will return requirements not met (false) (for install to be called) if an indicated app is already installed'
$customps1_injection_lines +='# even if installed, requirements are only not met if if its version is higher than indicated'
$customps1_injection_lines +='# note: this checks if the package itself is already installed. In this case you must show requirements_met, otherwise all installs will result in not applicable status'
$customps1_injection_lines +='# '
$customps1_injection_lines +='$app_to_find     = "Adobe Acrobat*"'
$customps1_injection_lines +='$app_ver_atleast = "0"'
$customps1_injection_lines +='#$app_ver_atleast = $app_ver_atleast'
$customps1_injection_lines +='#'
$customps1_injection_lines +='$apps = WingetList'
$customps1_injection_lines +='$apps_found = $apps | Where-Object Name -like $app_to_find'
$customps1_injection_lines +='ForEach ($app_found in $apps_found)'
$customps1_injection_lines +='{ # each found app'
$customps1_injection_lines +='    if ([version]$app_found.version -lt (GetVersionFromString $app_ver_atleast))'
$customps1_injection_lines +='    { # version is too low'
$customps1_injection_lines +='        WriteLog "$($app_found.name) v$($app_found.version): Old (not at least $($app_ver_atleast))"'
$customps1_injection_lines +='    } # version is too low'
$customps1_injection_lines +='    Else'
$customps1_injection_lines +='    { # version ok'
$customps1_injection_lines +='		if ($app_found.id -eq $IntuneApp.AppInstallName)'
$customps1_injection_lines +='		{ # exact app installed aleady'
$customps1_injection_lines +='			WriteLog "$($app_found.name) v$($app_found.version): Installed (matches package id $($IntuneApp.AppInstallName))"'
$customps1_injection_lines +='		}'
$customps1_injection_lines +='		Else'
$customps1_injection_lines +='		{ # this is a conflicting app so don''t install package'
$customps1_injection_lines +='		WriteLog "$($app_found.name) v$($app_found.version): OK (is at least v$($app_ver_atleast))"'
$customps1_injection_lines +='		if ($requirements_met) {$requirements_met = $false}'
$customps1_injection_lines +='	}'
$customps1_injection_lines +='} # version ok'
$customps1_injection_lines +='} # each found app'
$customps1_injection_lines +=''
$customps1_injection_lines +='#endregion Check if app installd'
$customps1_injection_lines +='#>'
$customps1_injection_lines +=''
$customps1_injection_lines +='#region Check for file'
$customps1_injection_lines +='#'
$customps1_injection_lines +='# this sample code will check for required files'
$customps1_injection_lines +='# if any of the files are found, it''s OK to install here'
...
$customps1_injection_lines +='$Filechecks +="$($env:USERPROFILE)\AppData\Roaming\Microsoft\Teams\desktop-config.json"'
$customps1_injection_lines +='$Filechecks +="$($env:USERPROFILE)\AppData\Local\Packages\MSTeams_8wekyb3d8bbwe\LocalCache\Microsoft\MSTeams\app_settings.json"'
...
$customps1_injection_lines +='}'
$customps1_injection_lines +='$requirements_met = $bOK'
