...
$intReturnCode, $strReturnMsg = WingetAction -WingetVerb "install" -WingetApp $WingetApp.App -SystemOrUser "system"
if ($intReturnCode -ne 0)
{   
    Write-Host "Machine scope install failed: $($strReturnMsg)"
    Write-Host "Trying again with no scope..." -NoNewline
    $intReturnCode, $strReturnMsg = WingetAction -WingetVerb "install" -WingetApp $WingetApp.App -SystemOrUser ""
}
Write-Host $strReturnMsg -ForegroundColor Yellow
