...
Function IntuneAppValues
{
	...
	$IntuneAppValues = @{}
    $IntuneAppValues.Add("AppName","Windows VPN (Rethinkit)-v107")
...
$customps1_injection_lines +='{'
$customps1_injection_lines +='	# This sample code will detect if there''s already an app installed of at least a certain version (using winget)'
$customps1_injection_lines +='    $app_to_find     = "Adobe Acrobat*"'
$customps1_injection_lines +='    #$app_ver_atleast = "24.2"'
$customps1_injection_lines +='    $app_ver_atleast = $IntuneApp.AppUninstallVersion'
$customps1_injection_lines +='    #'
$customps1_injection_lines +='    $apps = WingetList'
$customps1_injection_lines +='    $apps_c = $apps | Where-Object Name -like $app_to_find  # for match by name'
$customps1_injection_lines +='    #$apps_c = $apps | Where-Object Id -eq $app_to_find     # for exact match by id'
$customps1_injection_lines +='    ForEach ($app_c in $apps_c)'
$customps1_injection_lines +='    {'
$customps1_injection_lines +='        if ([version]$app_c.version -lt (GetVersionFromString $app_ver_atleast))'
$customps1_injection_lines +='        {'
$customps1_injection_lines +='            WriteLog "$($app_c.name) v$($app_c.version): Old (not at least $($IntuneApp.AppUninstallVersion))"'
$customps1_injection_lines +='        }'
$customps1_injection_lines +='        Else'
$customps1_injection_lines +='        {'
$customps1_injection_lines +='            WriteLog "$($app_c.name) v$($app_c.version): OK (is at least $($IntuneApp.AppUninstallVersion))"'
$customps1_injection_lines +='            if (-not $app_detected) {$app_detected = $true}'
$customps1_injection_lines +='        }'
$customps1_injection_lines +='    }'
...
$customps1_injection_lines +=''
$customps1_injection_lines +='$found = $false'
$customps1_injection_lines +='if ($Contents -ne '''') {'
$customps1_injection_lines +='    $conninfo = Get-VpnConnection -Name $Contents -ErrorAction Ignore'
$customps1_injection_lines +='    if ($conninfo) { #has vpn'
$customps1_injection_lines +='        $found = $true'
$customps1_injection_lines +='    }'
$customps1_injection_lines +='} # has contents'
$customps1_injection_lines +='if ($found){'
