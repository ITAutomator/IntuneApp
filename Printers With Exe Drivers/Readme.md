...
`PrinterSetupExe.ps1` in this package adds the EXE based printers listed in `PrinterSetupExe PrintersToAdd.csv` and the .exe installers in the `\Drivers` folder.
It also removes any printers listed in `PrinterSetupExe PrintersToRemove.csv` (if found)

The program uses three key components, all added via the Printer Manager main menu:

1. **PrinterSetupExe PrintersToAdd.csv**  
...
2. **PrinterSetupExe PrintersToRemove.csv**  
   An (optional) list of obsolete printers to remove from PCs.  

3. **IntuneApp/Drivers**  
   A folder of the .exe files used to install the added printers.  
   The .exe installers run as a service. They can display things, but must NOT stop for any reason and must close themselves upon completion.  
